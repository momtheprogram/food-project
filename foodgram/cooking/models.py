...
class Recipe(models.Model):
    author = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='recipes', db_index=True, verbose_name='Автор рецепта'
    )
    name = models.CharField(max_length=200, verbose_name='Название блюда')
    image = models.ImageField(upload_to='recipes/images', verbose_name='Фото блюда')
    text = models.TextField(verbose_name='Рецепт')
    ingredients = models.ManyToManyField(
        Ingredient, blank=False, through='IngredientQuantity', verbose_name='Ингредиенты'
    )
    tags = models.ManyToManyField(Tag, verbose_name='Тег', )
    cooking_time = models.IntegerField(
        verbose_name='Время приготовления', default=15, validators=[MinValueValidator(1)]
    )
    carts = models.ManyToManyField(User, through='Cart', related_name='recipes_in_cart')
...
class Favorite(models.Model):
    ...
    class Meta:
        ...
        constraints = [models.UniqueConstraint(fields=['user', 'recipe'], name='favorites_recipes_user_recipe_unique')]


class Cart(models.Model):
    recipe = models.ForeignKey(Recipe, on_delete=models.CASCADE, related_name='in_cart', verbose_name='Рецепт')
